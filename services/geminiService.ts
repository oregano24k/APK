...
export async function generateConversionSteps(repoUrl: string, androidVersion: string, selectedOS: 'macos_linux' | 'windows'): Promise<Step[]> {
  // Fix: Escaped all literal backticks (`) and dollar-sign-braces (${) in the prompt string to prevent them from being interpreted as template literal syntax.
  const prompt = `
    ...
    0.  **Limpieza Previa (Opcional pero Recomendado)**:
        - **title**: "Paso 0: Preparar un Lienzo Limpio"
        - **explanation**: "Si has intentado esto antes y encontraste errores, este paso es crucial. Limpiaremos cualquier configuración o carpeta antigua para empezar de cero y evitar conflictos. Es opcional, pero muy recomendado para asegurar que todo funcione bien."
        - **actions**: ${selectedOS === 'macos_linux'
            ? `[
                {"label": "Borrar Carpeta de Proyecto Anterior", "type": "command", "value": "rm -rf mi-app-web"},
                {"label": "Revisar/Limpiar .zshrc", "type": "command", "value": "nano ~/.zshrc"},
                {"label": "Revisar/Limpiar .bash_profile", "type": "command", "value": "nano ~/.bash_profile"}
               ]`
            : `[
                {"label": "Borrar Carpeta de Proyecto Anterior", "type": "command", "value": "rmdir /s /q mi-app-web"}
               ]`
        }
        - **details**: "${selectedOS === 'macos_linux'
            ? `--- 1. Borrar Carpeta de Proyecto Anterior ---\\nEl comando 'rm -rf mi-app-web' eliminará la carpeta del proyecto si la creaste en un intento anterior. **Aviso**: Este comando es potente y borra la carpeta permanentemente.\\n--- 2. Revisar Archivos de Configuración ---\\nUsa los botones para abrir tu archivo de configuración de shell (' .zshrc' para ZSH, '.bash_profile' para Bash). Una vez dentro, busca CUALQUIER línea de código que contenga 'ANDROID_HOME' o 'JAVA_HOME' y **bórrala por completo**. Esto es vital para evitar configuraciones duplicadas o incorrectas que causan errores. Guarda el archivo (Ctrl+O) y ciérralo (Ctrl+X) antes de continuar con el Paso 1."`
            : `--- 1. Borrar Carpeta de Proyecto Anterior ---\\nEjecuta este comando en tu terminal para eliminar la carpeta del proyecto si la creaste en un intento anterior. **Aviso**: Este comando es potente y borra la carpeta permanentemente.\\n--- 2. Limpiar Variables de Entorno Antiguas ---\\nSigue los mismos pasos del Paso 3 para 'Abrir la Configuración Avanzada del Sistema'. Una vez en la ventana de 'Variables de entorno', busca en 'Variables del sistema' cualquier variable llamada 'ANDROID_HOME' o 'JAVA_HOME'. Si existen, selecciónalas y **haz clic en 'Eliminar'**. Además, edita la variable 'Path' y elimina cualquier línea que haga referencia a la carpeta de Android SDK. Esto asegura que no queden restos de configuraciones antiguas."`
        }"
